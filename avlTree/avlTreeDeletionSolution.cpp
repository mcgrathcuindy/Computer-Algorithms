...
// modified subtree.  
Node* deleteNode(Node* root, int key/*, int *count*/)  
{  
...
int nodesGreaterThan(Node* node, int k) {
  if (node == NULL) {
    return 0;
  }

  int countLeft = nodesGreaterThan(node->left, k);
  int countRight = nodesGreaterThan(node->right, k);

  return (node->key > k ? 1 : 0) + countLeft + countRight;
}
// Function to find 
// minimum number of nodes 
//in avl tree of height height
...
Node *root = NULL;  
    ...
    */
    ...
    cout << "\n\tSorted array portion" << endl;
    // Assuming the array is sorted 
    int arr[] = { 1, 2, 3, 4, 5, 6, 7 }; 
    int n = sizeof(arr) / sizeof(arr[0]); 
  
    /* Convert List to AVL tree */
    Node* newRoot = sortedArrayToBST(arr, 0, n - 1); 
    ...
    cout << nodesGreaterThan(newRoot,2) << endl;
    return 0; 
}  